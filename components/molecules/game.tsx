...
const Game = () => {
  ...
  return (
    <>
      <div className="quiz flex flex-col xl:justify-center xl:items-center gap-2 lg:px-6 w-full max-h-96 lg:mt-16 xl:mt-0 xl:max-h-full">
        <AnimatePresence
          initial={false}
          mode="wait"
        ...
        >
          {shouldAnimateQuestion && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              key={currentQuestion}
              className="quiz flex flex-col gap-4"
            >
              <p className="quiz text-black italic xs:text-sm md:text-md text-[var(--gray-navy)] dark:text-[var(--light-blue)] xl:text-xl" >
                Асуулт {questions.length}-с  {currentQuestion + 1}  
              </p>
        ...
        >
          {shouldAnimateQuestion && (
            <MotionDiv
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="quiz mt-2 lg:mt-auto xl:mt-20 w-full "
      ...
      >
        {shouldAnimateAnswers && (
          <MotionDiv
            ...
            exit={{ opacity: 0, y: 20 }}
            className="quiz flex flex-col gap-y-4 justify-center w-full"
            key={currentQuestion} // Asegúrate de proporcionar una clave única para que React maneje las animaciones correctamente
          >
            <Answers
